...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          codeia_user_profile: this._safeGetItem('codeia_user_profile'),
          codeia_settings: this._safeGetItem('codeia_settings'),
          codeia_profile: this._safeGetItem('codeia_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('codeia_viewed_features'),
          developer_mode: this._safeGetItem('codeia_developer_mode'),
        ...
        ui: {
          // Tab configuration
          codeia_tab_configuration: this._safeGetItem('codeia_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('codeia_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('codeia_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          codeia_read_logs: this._safeGetItem('codeia_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('codeia_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('codeia_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.codeia_tab_configuration) {
        try {
          this._safeSetItem('codeia_tab_configuration', data.ui.codeia_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'codeia_acknowledged_debug_issues',
        'codeia_acknowledged_connection_issue',
        'error_logs',
        'codeia_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('codeia_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
